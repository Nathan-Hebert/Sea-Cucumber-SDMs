...
#takes a row vector of extracted BNAM values and
#returns an ordered and coherent time series
...
###FUNCTION TO CREATE RASTER STACK OF MONTHLY AVERAGES for 2000 to 2019 (and standard deviations)###
...
average_min_max_range_calc <- function(file_subset, years)
{
  ...
  for (i in 1:length(years))
  {
    min_raster_list[[i]] <- calc(stack(file_subset[grep(paste(years[i], ".asc", sep = ""), file_subset)]), fun = min)
    max_raster_list[[i]] <- calc(stack(file_subset[grep(paste(years[i], ".asc", sep = ""), file_subset)]), fun = max)
...
#helper function for calculating CPUE performance metrics
CPUE_metrics <- function(residuals)
{
  metrics <- list()
...
#helper function for calculating presence performance metrics
presence_metrics <- function(response, prediction_prob)
{
  metrics <- list()
  residuals <- response - prediction_prob
  metrics$auc <- auc(response, prediction_prob)
  metrics$brier <- mean((residuals)^2)
  return(metrics)
}

#make partial effects plot for GLMMs
#let a and be the index for the variable's terms, and let original_vector be the variables original training vector
effects_plot <- function(model, a, b = NULL, original_vector,..., line_color = "black", ci.color = "light blue", starve = F)
{
  ...
  else
  {
    if(starve == T)
    {
      ...
      se <- sqrt(x^2*model@tracing@parameter_covariance[a,a]+x^4*model@tracing@parameter_covariance[b,b]+2*x^3*model@tracing@parameter_covariance[a,b]) 
    }
    else
    {
...
##################################################MANAGEMENT APPLICATIONS HELPER FUNCTIONS#################################

#turn GMRF or starve predictions into a raster
...
#turn GAM predictions into a raster
