...
#project the rasters
raster_data_BtmTemp <- projectRaster(raster_data_BtmTemp, crs = "+proj=utm +zone=20T +datum=WGS84 +units=km")
raster_data_BtmSalinity <- projectRaster(raster_data_BtmSalinity, crs = "+proj=utm +zone=20T +datum=WGS84 +units=km")
raster_data_BtmStress <- projectRaster(raster_data_BtmStress, crs = "+proj=utm +zone=20T +datum=WGS84 +units=km")
RangeStress <- projectRaster(RangeStress, crs = "+proj=utm +zone=20T +datum=WGS84 +units=km")
RangeTemp <- projectRaster(RangeTemp, crs = "+proj=utm +zone=20T +datum=WGS84 +units=km")
RangeSalinity <- projectRaster(RangeSalinity, crs = "+proj=utm +zone=20T +datum=WGS84 +units=km")
...
#create a raster_data of predictor values for each year and convert the raster_datas into a data frame...
#for 2019 only, change 1:20 to 20:20
raster_data_df_timeseries <- data.frame()
for (i in 1:20)
{
  pred_raster_data <- stack(raster_data_BtmTemp_timeseries[[i]], raster_data_BtmSalinity_timeseries[[i]], DEM_timeseries, Easterness_timeseries, 
                      Northerness_timeseries, Slope_timeseries, RDMV_timeseries, snowcrab_ind_timeseries, 
                      raster_data_BtmStress_timeseries[[i]], RangeTemp_timeseries, RangeStress_timeseries,
                      RangeSalinity_timeseries)
...
#aggregate DEM files
aggregate_factor = 25
DEM_2019 <- aggregate(DEM, fact = aggregate_factor, na.rm = FALSE)
Easterness_2019 <- aggregate(Easterness, fact = aggregate_factor)
Northerness_2019 <- aggregate(Northerness, fact = aggregate_factor)
Slope_2019 <- aggregate(Slope, fact = aggregate_factor)
RDMV_2019 <- aggregate(RDMV, fact = aggregate_factor)

#make BNAM rasters comparable to DEM
raster_data_BtmTemp_2019 <- resample(raster_data_BtmTemp[[20]], DEM_2019)
raster_data_BtmSalinity_2019 <- resample(raster_data_BtmSalinity[[20]], DEM_2019)
raster_data_BtmStress_2019 <- resample(raster_data_BtmStress[[20]], DEM_2019)
RangeStress_2019 <- resample(RangeStress, DEM_2019)
RangeTemp_2019 <- resample(RangeTemp, DEM_2019)
RangeSalinity_2019 <- resample(RangeSalinity, DEM_2019)

#create a raster for snowcrab indicator
snowcrab_ind_2019 <- setValues(DEM_2019, TRUE)

#convert data into data frame
pred_raster_data <- stack(raster_data_BtmTemp_2019, raster_data_BtmSalinity_2019, DEM_2019, Easterness_2019, 
                    Northerness_2019, Slope_2019, RDMV_2019, snowcrab_ind_2019, 
                    raster_data_BtmStress_2019, RangeTemp_2019, RangeStress_2019, RangeSalinity_2019)
names(pred_raster_data) <- c("BtmTempBNAM","BtmSalinityBNAM","DEM", "DEM_Easterness",
                       "DEM_Northerness","DEM_Slope","DEM_RDMV", "snowcrab", "BtmStressBNAM",
                       "RangeTemp", "RangeStress","RangeSalinity")
raster_data_df_2019 <- as.data.frame(na.omit(rasterToPoints(pred_raster_data)))
raster_data_df_2019$year <- 2019
raster_data_df_2019$UTMX <- raster_data_df_2019$x
raster_data_df_2019$UTMY <- raster_data_df_2019$y
#trick to remove NA's caused by log
raster_data_df_2019$log_depth <- log(-raster_data_df_2019$DEM)
raster_data_df_2019 <- na.omit(raster_data_df_2019)

################################WRITE TO CSV################################

write.csv(raster_data_df_timeseries, paste(getwd(),"/Data/raster_data_df_timeseries.csv", sep = ""))
write.csv(raster_data_df_2019, paste(getwd(),"/Data/raster_data_df_2019.csv", sep = ""))